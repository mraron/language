...
func (cpp11) InsecureCompile(wd string, r io.Reader, w io.Writer, e io.Writer) (error) {
	cmd := exec.Command("g++","-std=c++11", "-x", "c++", "-o", "/dev/stdout", "-")
...
func (cpp11) Compile(s language.Sandbox, r io.Reader, w io.Writer, e io.Writer) error {
	...
	if err := s.SetMaxProcesses(-1).Env().TimeLimit(10 * time.Second).MemoryLimit(256000).Stdin(r).Stdout(bin).Stderr(e).WorkingDirectory("/tmp").Run("/usr/bin/g++ -std=c++11 -static -DONLINE_JUDGE -x c++ -o /dev/stdout -", false); err != nil {
		e.Write(bin.Bytes())
		return err
	}
